...
struct inode_disk {
	cluster_t start;                	/* First data sector. */
	off_t length;                       /* File size in bytes. */
	unsigned is_dir;					/* 0: file, 1: dir. */
...
inode_write_at (struct inode *inode, const void *buffer_, off_t size,
	...
	if (sector_idx == -1) {
		...
		for (int i = 0; i < sectors_new; i++) {
			cluster_t clst_new = fat_create_chain (cluster_EOF);
			if (clst_new != 0) {
				cluster_EOF = clst_new;
				static char zeros[DISK_SECTOR_SIZE];
...
/* Returns whether inode is dir. */
bool
inode_is_dir (const struct inode *inode) {
	return inode->data.is_dir == 1 ? true : false;
}